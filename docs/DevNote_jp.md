...
|delegate|arg0:original|arg1:weapon|arg2:num|return|
|-|-|-|-|-|
...
文字列補間はIL上は "{0}/{1}" のようなインデックス付きのフォーマットに過ぎないので、 "{1}分の{0}" のように単純な文字列置換で翻訳するのは可能だが、以下のような場合は破綻する：

"{0}/{1}" のフォーマット結果が "1/12" であるが、
バージョンアップにより "{0} {1}/{2}" となりフォーマット結果が "撃破数 1/12" などとなると、
"{1}分の{0}" のようにフォーマットしていたものは "1分の撃破数" という補完結果になってしまう。

どうやってバージョンアップ耐性をつけられるだろうか？

- "{0}/{1}" を検索して置換するなら、 "{0} {1}/{2}" はマッチしないので翻訳自体が無効になる。
    - 補完の数が変わらず "{0}, current:{1}" のようになるとどうか？これもマッチしないのでOK。
- 本体が "{creepName} {creepCount}" という文字列補完を "{creepCount} {creepName}" というふうに順序変更した場合はどうなるか？この場合は翻訳後の文字列も順序が変わることが自然なので問題ないかもしれない。